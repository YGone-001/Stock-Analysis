...
public partial class TurtleViewModel : ObservableObject
{
    ...
    private string _logText = "⚡ 海龟引擎已就绪！已连接股票数据网关，请点击【接口诊断】查看原始数据。\n";
    ...
    private async Task Test()
    {
        ...
        try
        {
			IStockDataGateway gateway = _mainVm.DataGateway ?? throw new InvalidOperationException("Stock-data gateway is unavailable.");
			string text = await gateway.GetEastMoneyAsync(new Uri("https://push2his.eastmoney.com/api/qt/stock/kline/get?secid=0.000001&klt=101&fqt=1&lmt=10&fields2=f51,f52,f53,f54,f55,f56,f57,f58,f59,f60,f61"));
            if (string.IsNullOrWhiteSpace(text))
            {
                AppendLog("❌ [诊断结果] 股票数据网关返回了空字符串！");
                return;
            }
